...
- **`.github/workflows/release.yaml`** (`name: release`; on `v*` tag push / manual): self-hosted
  ...
  `v*` tag does.
- **Env config** (two levels): `.env.example` holds ONLY environment-level vars — values that
  change between deploys, secrets, and compose infra wiring (gateway ports, db, `CORS_ORIGINS`,
  `LOG_LEVEL`, `AUTH_COOKIE_SECURE`, the per-deploy endpoints/TLS, 2 frontend `VITE_*`). App-level
  tuning knobs (session times, chat/agency/analytics/similarity timeouts and windows, app identity)
  are NOT in `.env.example`; they keep one value across every deploy and live only as `app/config.py`
  defaults, still env-overridable if a deploy needs it. Every var has a code default, so `.env` can be
  empty for dev; prod `.env` is generated by `release.yaml` from secrets. Prod-must-override vars
  (marked inline in `.env.example`): `ENV=production`, `DATABASE_URL`, `OPENROUTER_API_KEY`,
  `ONECHAT_BASE_URL`, `MCP_ENDPOINT_URL`, `PARSE_SPEC_API_KEY`. Auth is session-cookie based —
  there is no `JWT_SECRET` (the stale `backend/.env.example` with dead JWT vars was removed in the
  env-file merge). **CORS is wide-open by design**: `CORS_ORIGINS` defaults to `["*"]` with
  `allow_credentials=True` (Starlette reflects any origin); cross-site exposure is mitigated by
  `SameSite=Lax` session cookies (browsers don't attach them to cross-site XHR) and header-only API
  keys. The earlier `assert_production_config` wildcard guard was deliberately removed (see WS/open-CORS
  entry below). `compose.yaml` wires `ENV` + `CORS_ORIGINS` into the backend `environment:` block so an
  operator *can* restrict CORS with an explicit list, but no code enforces it.
...
- **CI was red on `main`**: `comment-cleanup-phase3` merge → `ci` → failure. Root cause was a
  truncated action pin in `.github/workflows/ci.yaml`: `pnpm/action-setup@0977fd99725f1dbb4e90d06cc86`
  (28 chars) — GitHub requires the full 40-char commit SHA and could not resolve it. Fixed to the
  verified full SHA `0977fd99725f1db4007ccb2928dbb4e90d06cc86` (v6.0.10). All four CI action SHAs now
  resolve.
- **`.env.example` carried a false claim**: it documented a `assert_production_config` production
  guard that rejects wildcard CORS — but no such function exists in `backend/app/`. The guard
  *did* exist once and was **deliberately removed** (see the "WS handshake fix + open CORS" entry:
  CORS is wide-open by design, mitigated by SameSite=Lax cookies + header API keys). The stale
  comment was the leftover. Corrected `.env.example` to describe the actual wide-open-CORS design
  and its mitigations. No guard was reintroduced — the documented design decision stands.
- **`compose.yaml` now wires `ENV` + `CORS_ORIGINS`** into the backend `environment:` block
  (previously only `DATABASE_URL` + `OPENROUTER_API_KEY` reached the container, so `.env` overrides
  for those two were silent no-ops). An operator can now restrict CORS with an explicit list; no
  code enforces it.
- **`compose.override.yaml`** stale comment fixed: referenced the old `deploy.yml` (now
  `release.yaml`).
