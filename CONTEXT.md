...
- **`.github/workflows/release.yaml`** (`name: release`; on `v*` tag push / manual): self-hosted
  ...
  `v*` tag does.
- **Env config**: a single `.env.example` documents every var (gateway, db, 60 backend `Settings`
  fields in `app/config.py`, 2 frontend `VITE_*`). Every var has a code default, so `.env` can be
