...
- **Env config**: a single `.env.example` documents every var (gateway, db, 60 backend `Settings`
  ...
  (marked inline in `.env.example`): `ENV=production`, `DATABASE_URL`, `CORS_ORIGINS` (not `"*"` in
  prod), `OPENROUTER_API_KEY`, `ONECHAT_BASE_URL`, `MCP_ENDPOINT_URL`, `PARSE_SPEC_API_KEY`. Auth is
  session-cookie based — there is no `JWT_SECRET` (the stale `backend/.env.example` with dead JWT
  vars was removed in the env-file merge).
