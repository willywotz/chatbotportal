...
async def save_turn(
    *,
    ...
    The caller owns the transaction boundary (its `session`'s commit) — this
    function only flushes, it never opens or commits a transaction itself.
    """
    ...
    if conv is None:
        conv = await conversation_repo.create(
            ...
            status=status,
            message_count=2,
            response_time=str(response_time),  # Conversation.response_time is a string column
        ...
        if not succeeded:
            ...
            conv.status = "failed"
        await conversation_repo.save(session, conv)
    user_msg = await message_repo.create(
        session, conversation_id=conversation_id, role="user", content=query, category=category,
    )
