...
async def test_init_db_runs_migrations_then_seeds(monkeypatch):
    ...
    monkeypatch.setattr("app.features.identity.oidc.keys.ensure_signing_key", _fake_ensure_key)
    monkeypatch.setattr("app.features.llm.services.seed.seed_llm_defaults", _fake_seed)
    monkeypatch.setattr("app.features.identity.services.user_seed.seed_admin_user", _fake_seed_admin)
    await db.init_db()
